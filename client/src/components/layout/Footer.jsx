...
const footerLinks = {
  Platform: [
    ...
    { label: "Hackathons", href: "/#programs" },
...
export default function Footer() {
  return (
    <footer id="about" className="relative border-t border-white/5 bg-slate-950/80 mt-24">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-2 mb-4">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-fuchsia-500 text-white font-bold">
                ER
              </span>
              <span className="font-bold text-xl text-white">Event Radar</span>
            </Link>
            <p className="text-slate-400 text-sm leading-relaxed max-w-sm mb-6">
              India&apos;s premium campus event discovery platform. Find hackathons, fests, workshops, and competitions from top colleges — all in one place.
            </p>
            <div className="flex gap-3">
              {socials.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="p-2.5 rounded-xl glass-light text-slate-400 hover:text-white hover:border-indigo-500/30 transition-all duration-300"
                >
        ...
        <div className="mt-14 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-500">
          <p>© {new Date().getFullYear()} Event Radar. All rights reserved.</p>
